			...
			<FIELDSET>
				...
				<TABLE class="form">
					...
					<TBODY>
						<TR>
							<TH class="must">
								<LABEL for="{generate-id(subject)}">主旨</LABEL>
							</TH>
							<TD>
								<INPUT id="{generate-id(subject)}" name="subject" required="" type="text" value="{subject}"/>
							</TD>
							<TD>必填。</TD>
						</TR>
						<TR>
							<TH class="must">
								<LABEL for="wysiwyg">內容</LABEL>
							</TH>
							<TD>
								<TEXTAREA id="wysiwyg" cols="1" name="hyperTextMarkupLanguage" required="" rows="1">
									<xsl:value-of select="hyperTextMarkupLanguage" disable-output-escaping="yes"/>
								</TEXTAREA>
							</TD>
							<TD>必填。</TD>
						</TR>
						<TR>
							<TH class="must">
								<LABEL for="{generate-id(when)}">發佈日期</LABEL>
							</TH>
							<TD>
								<INPUT class="dP" id="{generate-id(when)}" name="when" readonly="" required="" type="text" value="{when}"/>
							</TD>
							<TD>必選。</TD>
						</TR>
						<TR>
