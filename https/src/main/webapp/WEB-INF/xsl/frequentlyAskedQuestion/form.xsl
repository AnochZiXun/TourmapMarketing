			...
			<FIELDSET>
				...
				<TABLE class="form">
					...
					<TBODY>
						<TR>
							<TH class="must">
								<LABEL for="{generate-id(question)}">問題</LABEL>
							</TH>
							<TD>
								<INPUT id="{generate-id(question)}" name="question" required="" type="text" value="{question}"/>
							</TD>
							<TD>必填。</TD>
						</TR>
						<TR>
							<TH class="must">
								<LABEL for="{generate-id(answer)}">回答</LABEL>
							</TH>
							<TD>
								<TEXTAREA id="{generate-id(answer)}" cols="1" name="answer" required="" rows="1">
